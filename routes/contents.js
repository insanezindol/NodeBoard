...
var BoardContents = require('../models/boardSchema'); //db를 사용하기 위한 변수
var fs = require('fs');
var multer = require('multer'); // 파일 저장을 위한  multer
var upload = multer({dest:'D:/_Storage/board_temp/'}); // multer 경로 설정, 파일이 업로드 되면 먼저 임시 폴더로 가서 저장됨
...
router.get('/download/:path', function(req, res){
    ...
    res.download('D:/_Storage/board_dest/'+path, path);
    console.log(path);
});
...
function renameUploadFile(itemId,upFile){
    ...
    for (var i = 0; i < upFile.length; i++) {
        ...
        fsName [i] = "D:/_Storage/board_dest/"+upFile[i].originalname;
    }
...
function isSaved(upFile) {
    ...
    if(savedFile != null) { // 파일 존재시 -> tmp폴더에 파일 저장여부 확인 -> 있으면 저장, 없으면 에러메시지
        for (var i = 0; i < savedFile.length; i++) {
            if(fs.statSync(savedFile[i].path).isFile()){ //fs 모듈을 사용해서 파일의 존재 여부를 확인한다.
